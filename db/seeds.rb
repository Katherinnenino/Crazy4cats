...
until User.count == 20 do
    User.create(email: "test#(i)@email.cl", password: "123456", "password_confirmation": "123456", photo: Faker::Avatar.unique.image, name: Faker::Name.unique.name)
    i +=1
end
users = User.all
...
until Reaction.count == 100 do
    ...
    Reaction.create(post_id: post.sample.id, 
        user_id:users.sample.id, 
        kind: kinds.sample, 
        )
    end
